...
export const LoginPage: React.FC<Props> = ({ onNavigate }) => {
  ...
  const handleGoogleSuccess = async (profile: { name: string; email: string; avatar: string }) => {
    try {
      ...
      toast.success(`Bem-vindo, ${profile.name}! Conta Google conectada.`);
      onNavigate('dashboard');
    } catch {
    ...
    <div className="relative min-h-[90vh] flex flex-col items-center justify-center px-4 py-12 overflow-hidden">
      ...
      <div className="relative z-10 text-center max-w-2xl mx-auto mb-8 space-y-3 animate-slide-up">
        ...
        <p className="text-xs sm:text-sm text-zinc-300/80 max-w-md mx-auto leading-relaxed">
          Encontre comércios reais sem site no Google Maps e apresente a demonstração perfeita em segundos.
        </p>
      </div>
      ...
      <div className="relative z-10 w-full max-w-md p-8 rounded-3xl border-beam-card space-y-6 animate-slide-up text-center">
        <div className="flex flex-col items-center space-y-2">
          ...
          <p className="text-xs text-zinc-400">Entre com sua conta Google oficial para acessar seus leads e projetos</p>
        </div>

        {/* Google Exclusive One-Click Login Button */}
        <div className="space-y-4 pt-2">
          ...
          >
            ...
            <span>{googleLoading ? 'Conectando ao Google...' : 'Entrar com o Google'}</span>
          </button>
        ...
        <div className="pt-4 border-t border-zinc-800/80 space-y-2 text-left text-xs text-zinc-400">
          <div className="flex items-center gap-2">
            ...
            <span>Acesso instantâneo sem necessidade de criar senha</span>
          </div>
          <div className="flex items-center gap-2">
            <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400 shrink-0" />
            <span>Sincronização em nuvem no Supabase</span>
          </div>
          <div className="flex items-center gap-2">
            <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400 shrink-0" />
            <span>Proteção contra e-mails falsos e clonagem de contas</span>
          </div>
        </div>
      </div>
